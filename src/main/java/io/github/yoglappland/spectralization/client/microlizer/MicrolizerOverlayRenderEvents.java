...
public final class MicrolizerOverlayRenderEvents {
    ...
    private static final int SPARK_COUNT = 72;
    ...
    private static final double ORBITING_SPARK_DISK_INNER_SCALE = 0.18D;
    private static final double ORBITING_SPARK_DISK_OUTER_SCALE = 0.94D;
    private static final double ORBITING_SPARK_RIM_CHANCE = 0.22D;
    ...
    @SubscribeEvent
    public static void renderMicrolizerConnections(RenderLevelStageEvent event) {
        ...
        var segments = ClientMicrolizerOverlayCache.activeSegments();
        var workArea = ClientMicrolizerWorkAreaOverlayCache.activeWorkArea();
        var animations = ClientMicrolizerAnimationCache.activeAnimations(
        ...
        if (hudVisible) {
            for (MicrolizerOverlayPayload.Segment segment : segments) {
        ...
        for (ClientMicrolizerAnimationCache.ActiveAnimation animation : animations) {
    ...
    private static void renderSegment(VertexConsumer consumer, PoseStack.Pose pose, MicrolizerOverlayPayload.Segment segment) {
    ...
    private static void renderWorkArea(
            VertexConsumer consumer,
            PoseStack.Pose pose,
            ClientMicrolizerWorkAreaOverlayCache.WorkArea area
    ) {
    ...
    private static void renderCompressionProjection(
            ...
            PoseStack poseStack,
            PoseStack.Pose pose,
            ClientMicrolizerAnimationCache.ActiveAnimation animation
    ) {
        ...
        if (age < MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS) {
            renderPhaseSeparation(consumer, pose, animation, centerX, centerY, centerZ);
        }

        if (age >= MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS
                && age < MicrolizerAnimationPayload.MERGE_AT_TICKS) {
        ...
        if (age >= MicrolizerAnimationPayload.MERGE_AT_TICKS) {
    ...
    private static void renderCompressionField(
        ...
        double fade = 1.0D - smoothRange(
                animation.age(),
                MicrolizerAnimationPayload.MERGE_AT_TICKS,
        ...
        if (animation.age() < MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS) {
            double scan = smootherStep(smoothRange(
                    animation.age(),
                    4.0D,
                    MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS - 3.0D
            ));
        ...
        double flash = 1.0D - smoothRange(
                Math.abs(animation.age() - MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS),
                0.0D,
                4.5D
        );
    ...
    private static void renderPhaseSeparation(
        ...
        double fieldProgress = smootherStep(smoothRange(
                animation.age(),
                10.0D,
                MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS - 2.0D
        ));
        ...
        double clearFade = 1.0D - smoothRange(
                animation.age(),
                MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS - 5.0D,
                MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS
        ) * 0.35D;
    ...
    private static int renderCollapsingSnapshotBlocks(
        ...
        for (ClientMicrolizerAnimationCache.ProjectedBlockSnapshot snapshot : animation.projectedBlockSnapshots()) {
    ...
    private static void renderOrbitingSparks(
        ...
        for (int index = 0; index < SPARK_COUNT; index++) {
            ...
            double seedE = unitHash(animation.corePos().asLong() + index * 39229L + 109L);
            ...
            double diskRadiusScale = lerp(
                    ORBITING_SPARK_DISK_INNER_SCALE,
                    ORBITING_SPARK_DISK_OUTER_SCALE,
                    Math.sqrt(seedB)
            );
            if (seedE > 1.0D - ORBITING_SPARK_RIM_CHANCE) {
                diskRadiusScale = lerp(0.78D, 1.0D, seedB);
            }

            double diskRadius = baseRadius * diskRadiusScale;
            double verticalThickness = Math.max(0.34D, maxExtent * 0.24D);
            double verticalOffset = (seedC - 0.5D) * verticalThickness * lerp(0.62D, 1.0D, diskRadiusScale);
            double orbitRadius = Math.max(baseRadius * ORBITING_SPARK_DISK_INNER_SCALE, diskRadius);
            ...
            double radius = diskRadius * radialScale;
            ...
            double size = (0.11D + seedD * 0.12D + maxExtent * 0.015D)
                    ...
                    * lerp(0.82D, 1.08D, diskRadiusScale)
    ...
    private static void renderFinalParticleTracks(
        ...
        double finalSpan = Math.max(1.0D, animation.durationTicks() - MicrolizerAnimationPayload.MERGE_AT_TICKS);
        double burst = easeOutCubic(smoothRange(
                animation.age(),
                MicrolizerAnimationPayload.MERGE_AT_TICKS,
                MicrolizerAnimationPayload.MERGE_AT_TICKS + finalSpan * 0.72D
        ));
        double fade = 1.0D - smoothRange(
                animation.age(),
                MicrolizerAnimationPayload.MERGE_AT_TICKS + finalSpan * 0.28D,
                animation.durationTicks()
        );
    ...
    private static double orbitAngle(double age, double accelerationScale) {
        double slowEnd = MicrolizerAnimationPayload.SLOW_ORBIT_TICKS;
        double collapseStart = MicrolizerAnimationPayload.CLEAR_WORK_AREA_AT_TICKS;
        double mergeStart = MicrolizerAnimationPayload.MERGE_AT_TICKS;
    ...
    private MicrolizerOverlayRenderEvents() {
    }
}
