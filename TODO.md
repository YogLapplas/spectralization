...
- [~] 微缩机器第一版完成：anchor/core/io_port 框架、core UI、工作区域显示、压缩输出物品和压缩后光学端口映射已接入。
- [ ] 下一阶段主线：微缩化光路工作站、第一批可压缩机器结构和可配置 IO 面映射。

## 光学 Profile

- [x] `FrequencyKey = SpectralRegion + bin`。
- [x] 频率 bin 向量进入编译器：创造光源可以通过柱状频谱页输出最多 32 个活跃频率分量，solver 按活跃 lane 并行求解再合并读数，光谱仪可以读取同一套频率分量。
- [x] 环境光收集器使用平滑谱族 LUT，原版光源只通过环境采样进入散杂光，不作为全局内禀光源。
- [x] 创造光源默认频谱改为可见光白光连续谱，非可见波段默认为 0。
- [~] 频谱外部编辑接口占位：`SpectrumProfile` / `SpectrumBand` / editor override API 已预留，后续可由 Python 工具生成数据。
- [x] 功率 profile。
- [x] 相干 / 散杂通道。
...
- [~] 微缩机器 anchor、core、io_port 和 compacted_machine：框架验证、核心 UI、压缩输出与基础光学映射已接入。
...
- [~] 微缩机器压缩光路：内部 IO 口、环境光收集器、光纤接口和红宝石增益已能被折叠进压缩后方块的边界行为。
...
- [ ] 自定义机器通过光端口 IO。
- [ ] compacted_machine UI：原结构预览、每个面的 IO 映射配置和端口重命名。
