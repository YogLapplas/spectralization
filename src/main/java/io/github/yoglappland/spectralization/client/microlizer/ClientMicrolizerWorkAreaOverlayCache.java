...
public final class ClientMicrolizerWorkAreaOverlayCache {
    ...
    public static void toggle(BlockPos corePos, BlockPos min, BlockPos max) {
        ...
        if (activeWorkArea != null && activeWorkArea.corePos().equals(corePos)) {
            if (activeWorkArea.min().equals(min) && activeWorkArea.max().equals(max)) {
                activeWorkArea = null;
                return;
            }
    ...
    private ClientMicrolizerWorkAreaOverlayCache() {
    }
}
