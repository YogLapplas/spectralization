...
当前版本已经越过最早的验证阶段。第一代光学网络编译器、红宝石激光器、光纤、热力熔炼、基础光刻、掺杂、全息存储矩阵和微缩机器都已经有可玩的原型。接下来的目标是把这些系统收束成中期玩法：材料体系、透镜路线、超材料/光刻路线、远程机器交互和虚空存储总线。
...
- 光学材料和表面：红宝石块、银块、涂银玻璃、石英/硼硅/冕牌/燧石/重玻璃、硼砂、金/银涂料、砂纸、高级刷子、创造刷子和世界表面涂层。
- 红宝石激光器原型：萤石类激发源让相邻红宝石成为六向相干种子源，泵浦源提高红宝石单程增益，银块和涂银玻璃形成初级腔。
- 光纤激光器原型：作为电泵浦增益介质接入光学网络，输出适合光纤路线的相干光，并通过功率滑条控制电力消耗和增益。
- 光束和 spot 可视化：真理头盔、皮革帽、光纤卷和剪刀暂时承担 debug 可视化，光束与光斑共用频率到 RGB 的 LUT。
- 光纤系统第一版：接口、relay、自动寻路连接、光输入输出、并行光纤容量、剪断光纤和 Jade 连接数显示。
- 光热工业原型：光热发电机、环境光收集器、高级环境光收集器、热力熔炼炉、光学玻璃熔炼路线和熔融流体。
- 光刻与掺杂原型：基础光刻机、基础掩膜、电路板配方、太阳能掺杂器、维度掺杂器，以及 YAG、CaF2、SiO2、Si 等掺杂材料链。
...
这条规则也约束新功能：新光源、新传感器、新材料、新可视化和新机器行为必须优先接入拓扑编译器、缓存和 readout 层。主运行路径是 `world -> port graph -> material gain/saturation rewrite -> power solve -> readout`。旧 tracer、profile-state exact solver 和示例验证器可以保留为 reference / oracle / legacy compare，但不能反向改变 gameplay authority。

当前光学求解器遵循两条哲学：

- 无逐 tick 收敛：游戏求解期做一次有限代数求解，不把反馈环留给逐 tick 追光。
- 多等效：半径、发散、光纤耦合、孔径裁剪等几何敏感效应先折叠成边的 effective gain，再进入同一套线性系统。显示层可以解释 profile 变差，功率层只接受保守损耗。
- 材料非线性局部化：主动增益不再用全局稳定性预调度；增益介质自己提供单程增益、饱和功率和吸收过载，求解器只解由这些材料定律组成的有限分段仿射系统。
...
- [docs/SPOT_PROJECTION_ALGORITHM.md](docs/SPOT_PROJECTION_ALGORITHM.md)：光斑投影的权威算法、不变量、已知限制和优化约束。
- [docs/SPOT_PROJECTION_PERFORMANCE.md](docs/SPOT_PROJECTION_PERFORMANCE.md)：典型性能参数、阶段耗时、测试基线和优化比较协议。
- [docs/SPOT_PROJECTION_CHECKPOINT_2026-07-11.md](docs/SPOT_PROJECTION_CHECKPOINT_2026-07-11.md)：当前实现、自动测试、性能基线与下一阶段计划。
- [docs/TECHNICAL_WRITING_GUIDE.md](docs/TECHNICAL_WRITING_GUIDE.md)：技术写作规范，用于后续严肃文档。
- [docs/SPECTRAL_PARAMETERS_UI.md](docs/SPECTRAL_PARAMETERS_UI.md)：JEI 增益材料页、光谱仪、光谱颜色和材料谱线图的维护规则。
