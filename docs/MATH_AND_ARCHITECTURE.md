...
当前可见光第一版使用 32 个 bin。频率系统采用活跃 lane，而不是为每一个 bin 复制一整张世界图。几何层、拓扑层、
SCC、chord 和 solver plan 共享；数据层按 `FrequencyKey + CoherenceKind` 分 lane
求解。第一版只激活光源、增益介质或转换元件实际产生的频率，并受
`BeamPacket.MAX_COMPONENTS` 控制；当前上限为 32。每个 lane 用同一张 port graph 的频率视图求解：
边上保存每个频率的 sample gain，solver 对各 lane 独立运行；当 lane 数和图规模
达到阈值时，这些 lane 可以并行求解，随后回到主线程合并为总功率、相干功率和
可视化输出。可视化层使用统一的 `SpectralColorMap` LUT，把可见光 bin 映射到 RGB。
