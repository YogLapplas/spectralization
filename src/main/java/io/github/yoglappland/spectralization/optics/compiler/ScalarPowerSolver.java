...
public final class ScalarPowerSolver {
    ...
    public static ScalarPowerSolution solve(CompiledPortGraph graph, Map<PortGraphNode, Double> sourcePowersByNode) {
        ...
        if (graph.feedbackSccCount() == 0 && graph.beta1() == 0) {
            ScalarPowerSolution acyclicSolution = solveAcyclic(graph, nodeIds, source);

            if (acyclicSolution != null) {
                return acyclicSolution;
            }
        }
    ...
    private static ScalarPowerSolution solveAcyclic(
        ...
        return buildSolution(true, false, 1, 0.0, graph.nodes(), powers);
    }
